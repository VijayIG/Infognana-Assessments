...
class Invoice:
    ...
    def post_processing(
        ...
        for data_find in data_fetch:
            ...
            if data_find in ["supplieremail_data"] and data:
                data = data.replace(" ", "")
            ...
            elif data_find in ["invoicenumber_data", "deliverynote_data"] and data:
                regex_rule = r"[\w]*[-]*[\w]+[0-9\/]+[A-Za-z0-9\/_,.-]+"
                regex_patt = re.compile(regex_rule)
                matched = re.search(regex_patt, data)
                if matched:
                    value = matched.group(0).strip()
                    date_finder = search_dates(value)
                    if not date_finder:
                        single_line_data.setdefault(data_find, []).append(
                            [
                                value,
            ...
            elif data_find in ["supplierphone_data", "faxno_data"] and data:
        ...
        ending_chracter = ['Company',
                           'Ltd',
                            ...
                            'Industries',
                            'Associates']
        if "suppliername_data" in Multiline_data:
            data = Multiline_data['suppliername_data'][0][0]
            for end_ch in ending_chracter:
                if end_ch.lower() in data.lower():
                    idx = data.lower().index(end_ch.lower())
                    data  = data[:idx+len(end_ch)]
                    print(data)
                    break
            if data:
                Multiline_data['suppliername_data'][0][0] = data

        Currency_data = currency(ocr_words, Multiline_data, page_no)
        Vat_data = tax(Multiline_data, ocr_words, page_no)
        single_line_data.update(Multiline_data)
        single_line_data.update(Currency_data)
        single_line_data.update(Vat_data)
        return single_line_data

    def process_document(self):
        model_name = "invoice_global_model.pt"
